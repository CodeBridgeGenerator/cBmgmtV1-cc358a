...
const [subordinateIds, setsubordinateIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("staffinfo")
                        .find({ query: { $select: ["_id","supervisor"] } })
                        .then((data) => {
                        setsuperiorIds(data.data);
                        });
client.service("staffinfo")
                        .find({ query: { $select: ["_id","empno"] } })
                        .then((data) => {
                        setsubordinateIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = superiorFakerFactory(props.user,fakerCount,superiorIds,subordinateIds);
