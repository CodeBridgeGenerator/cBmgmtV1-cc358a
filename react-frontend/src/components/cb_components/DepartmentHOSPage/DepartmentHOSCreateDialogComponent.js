...
const [employees, setEmployees] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [departmentId,employees], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            departmentId: _entity?.departmentId?._id,employees: _entity?.employees?._id,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Section Heads updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Section Heads" });
        }
        setLoading(false);
    };
...
useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            setEmployees(res.data.map((e) => { return { name: e['fullname'], value: e._id }}));
                        })
                        .catch((error) => {
                            console.debug({ error });
...
const employeesOptions = employees.map((elem) => ({ name: elem.name, value: elem.value }));

    return (
        <Dialog header="Create Section Heads" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="departmentHOS-create-dialog-component">
