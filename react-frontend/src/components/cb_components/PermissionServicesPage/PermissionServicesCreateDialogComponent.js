...
const PermissionServicesCreateDialogComponent = (props) => {
  ...
  const validate = () => {
    ...
    switch (selectedOption) {
      case "roleId":
        if (!(_entity?.roleIds?.length > 0))
          errors.selection = "Please select at least one role.";
        break;
      case "positionId":
        if (!(_entity?.positionIds?.length > 0))
          errors.selection = "Please select at least one position.";
        break;
      case "profile":
        if (!(_entity?.profileIds?.length > 0))
          errors.selection = "Please select at least one profile.";
        break;
      default:
        break;
    }
  ...
  const onSave = async () => {
    ...
    try {
      ...
      props.alert({
        ...
        message:
          error.message ||
          "Failed to create permissions. Some combinations may already exist.",
      });
  ...
  useEffect(() => {
    ...
    client
      ...
      .find(query)
      .then((res) =>
        setProfiles(res.data.map((e) => ({ name: e.name, value: e._id }))),
      )
      .catch((err) =>
        props.alert({
          title: "Profiles",
    ...
    client
      ...
      .find(query)
      .then((res) =>
        setRoles(res.data.map((e) => ({ name: e.name, value: e._id }))),
      )
      .catch((err) =>
        props.alert({
          title: "Roles",
    ...
    client
      ...
      .find(query)
      .then((res) =>
        setPositions(res.data.map((e) => ({ name: e.name, value: e._id }))),
      )
      .catch((err) =>
        props.alert({
          title: "Positions",
      ...
      >
        <div className="col-12 md:col-6 field mt-5">
          ...
          <MultiSelect
            ...
            value={_entity?.services}
            options={serviceOptions}
            onChange={(e) => setValByKey("services", e.value)}
            filter
            placeholder="Select Services"
            display="chip"
          />
          {!_.isEmpty(error.services) && (
            <small className="p-error">{error.services}</small>
          )}
        </div>
        ...
        <div className="col-12 md:col-6 field mt-5">
          ...
          <div className="mt-2">
            {selectedOption === "profile" && (
              <MultiSelect
                id="profileIds"
                value={_entity?.profileIds}
                options={profiles}
                onChange={(e) => setValByKey("profileIds", e.value)}
                optionLabel="name"
                optionValue="value"
            ...
            {selectedOption === "roleId" && (
              <MultiSelect
                id="roleIds"
                value={_entity?.roleIds}
                options={roles}
                onChange={(e) => setValByKey("roleIds", e.value)}
                optionLabel="name"
                optionValue="value"
            ...
            {selectedOption === "positionId" && (
              <MultiSelect
                id="positionIds"
                value={_entity?.positionIds}
                options={positions}
                onChange={(e) => setValByKey("positionIds", e.value)}
                optionLabel="name"
                optionValue="value"
          ...
          {!_.isEmpty(error.selection) && (
            <small className="p-error">{error.selection}</small>
          )}
        </div>
        ...
        <div className="col-12 grid mt-3">
          {[
            ...
            <div
              className="col-6 md:col-3 field flex align-items-center"
              key={perm}
            >
              <Checkbox
                inputId={perm}
                checked={_entity?.[perm] || false}
                onChange={(e) => setValByKey(perm, e.checked)}
                className="mr-2"
              />
