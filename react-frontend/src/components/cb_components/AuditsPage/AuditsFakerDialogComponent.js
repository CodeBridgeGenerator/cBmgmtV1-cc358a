...
import auditsFakerFactory from "./auditsFakerFactory";
import { getSchemaValidationErrorsStrings } from "../../../utils";

const AuditsFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = auditsFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("audits").create(elem),
    );
...
export default connect(mapState, mapDispatch)(AuditsFakerDialogComponent);
