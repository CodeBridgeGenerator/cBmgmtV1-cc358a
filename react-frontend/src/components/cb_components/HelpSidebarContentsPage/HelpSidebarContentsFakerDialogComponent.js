...
import helpSidebarContentsFakerFactory from "./helpSidebarContentsFakerFactory";
...
const HelpSidebarContentsFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = helpSidebarContentsFakerFactory(props.user, fakerCount);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("helpSidebarContents").create(elem),
    );
