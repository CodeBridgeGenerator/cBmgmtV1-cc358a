...
import ProfileMenuDatatable from "./ProfileMenuDataTable";
...
import ProfileMenuFakerDialogComponent from "./ProfileMenuFakerDialogComponent";
import ProfileMenuSeederDialogComponent from "./ProfileMenuSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";
//import {generateSchema} from "../../../utils/utils"; // No longer needed
...
const ProfileMenuPage = (props) => {
  ...
  useEffect(() => {
    const simplifiedSchema = {
      data: [
        { field: "role", type: "ObjectId", ref: "roles" },
        { field: "position", type: "ObjectId", ref: "positions" },
        { field: "profile", type: "ObjectId", ref: "profiles" },
        { field: "user", type: "ObjectId", ref: "users" },
        { field: "company", type: "ObjectId", ref: "companies" },
        { field: "branch", type: "ObjectId", ref: "branches" },
        { field: "department", type: "ObjectId", ref: "departments" },
        { field: "section", type: "ObjectId", ref: "sections" },
        { field: "createdBy", type: "ObjectId", ref: "users" },
        { field: "updatedBy", type: "ObjectId", ref: "users" },
        // Add other fields from your schema *except* menuItems
      ],
    };

    const _fields = simplifiedSchema.data
      .map((field, i) => i > 15 && field.field)
      .filter(Boolean);
    setFields(simplifiedSchema.data);
  ...
  const deleteRow = async () => {
    try {
      await client.service("profileMenu").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("profileMenu").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="profileMenu-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-12" role="profileMenu-datatable">
          <ProfileMenuDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(ProfileMenuPage);
