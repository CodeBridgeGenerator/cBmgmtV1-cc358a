...
import profileMenuFakerFactory from "./ProfileMenuFakerFactory";
...
const ProfileMenuFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = profileMenuFakerFactory(props.user, fakerCount, userIds);

    setLoading(true);
    const promises = fakeData.map((elem) =>
      client.service("profileMenu").create(elem),
    );
...
export default connect(mapState, mapDispatch)(ProfileMenuFakerDialogComponent);
