...
import frontendsFakerFactory from "./frontendsFakerFactory";
...
const FrontendsFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("firebase")
                        .find({ query: { $select: ["_id","projectId"] } })
                        .then((data) => {
                        setfirebaseIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = frontendsFakerFactory(props.user,fakerCount,firebaseIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("frontends").create(elem));
...
export default connect(mapState, mapDispatch)(FrontendsFakerDialogComponent);
