...
import firebaseFakerFactory from "./firebaseFakerFactory";
...
const FirebaseFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = firebaseFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("firebase").create(elem));
...
export default connect(mapState, mapDispatch)(FirebaseFakerDialogComponent);
