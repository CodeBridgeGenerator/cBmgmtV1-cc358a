...
const FirebaseCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            projectId: _entity?.projectId,url: _entity?.url,customUrl: _entity?.customUrl,key: _entity?.key,env: _entity?.env,projectNumber: _entity?.projectNumber,webApiKey: _entity?.webApiKey,appId: _entity?.appId,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Firebase created successfully" });
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="projectNumber" className="w-full mb-3 p-inputtext-sm" value={_entity?.projectNumber} onChange={(e) => setValByKey("projectNumber", e.value)}  />
            </span>
            <small className="p-error">
