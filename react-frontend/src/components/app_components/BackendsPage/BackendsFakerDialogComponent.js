...
import backendsFakerFactory from "./backendsFakerFactory";
...
const BackendsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [frontendIds, setfrontendIds] = useState();
const [contractIds, setcontractIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("frontends")
                        .find({ query: { $select: ["_id","projectName"] } })
                        .then((data) => {
                        setfrontendIds(data.data);
                        });
client.service("contract")
                        .find({ query: { $select: ["_id","crm"] } })
                        .then((data) => {
                        setcontractIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = backendsFakerFactory(props.user,fakerCount,frontendIds,contractIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("backends").create(elem));
...
export default connect(mapState, mapDispatch)(BackendsFakerDialogComponent);
