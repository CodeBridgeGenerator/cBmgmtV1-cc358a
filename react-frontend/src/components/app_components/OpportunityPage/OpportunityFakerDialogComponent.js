...
import opportunityFakerFactory from "./opportunityFakerFactory";
...
const OpportunityFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = opportunityFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("opportunity").create(elem));
...
export default connect(mapState, mapDispatch)(OpportunityFakerDialogComponent);
