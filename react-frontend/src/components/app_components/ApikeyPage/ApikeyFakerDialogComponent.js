...
import apikeyFakerFactory from "./apikeyFakerFactory";
...
const ApikeyFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = apikeyFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("apikey").create(elem));
...
export default connect(mapState, mapDispatch)(ApikeyFakerDialogComponent);
