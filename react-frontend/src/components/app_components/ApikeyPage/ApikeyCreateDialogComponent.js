...
const ApikeyCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            apikey: _entity?.apikey,projectName: _entity?.projectName,requests: _entity?.requests,duration: _entity?.duration,serviceName: _entity?.serviceName,active: _entity?.active || false,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Apikey created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Apikey" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Apikey" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="apikey-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="requests" className="w-full mb-3 p-inputtext-sm" value={_entity?.requests} onChange={(e) => setValByKey("requests", e.value)}  />
            </span>
            <small className="p-error">
...
<div className="col-12 md:col-6 field flex">
            <span className="align-items-center">
                ...
                <Checkbox id="active" className="ml-3" checked={_entity?.active} onChange={(e) => setValByKey("active", e.checked)}  />
            </span>
            <small className="p-error">
