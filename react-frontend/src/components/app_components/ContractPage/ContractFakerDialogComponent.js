...
import contractFakerFactory from "./contractFakerFactory";
...
const ContractFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contractFakerFactory(props.user,fakerCount,crmIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contract").create(elem));
...
export default connect(mapState, mapDispatch)(ContractFakerDialogComponent);
