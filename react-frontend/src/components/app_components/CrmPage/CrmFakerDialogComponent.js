...
import crmFakerFactory from "./crmFakerFactory";
...
const CrmFakerDialogComponent = (props) => {
...
client.service("branches")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setpersonIds(data.data);
                        });
client.service("opportunity")
                        .find({ query: { $select: ["_id","states"] } })
                        .then((data) => {
                        setopportunityIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = crmFakerFactory(props.user,fakerCount,companyIds,personIds,opportunityIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("crm").create(elem));
...
export default connect(mapState, mapDispatch)(CrmFakerDialogComponent);
